...
document.getElementById('engineSelect').addEventListener('change', (e) => {
    ...
    ['3dmol', 'molstar', 'jsmol' , 'ngl'].forEach(eng => {
        ...
        if (eng === currentEngine) el.classList.replace('hidden-engine', 'active-engine');
        else el.classList.replace('active-engine', 'hidden-engine');
    });
...
document.getElementById('toggleUnknown').addEventListener('change', () => {
    if (modifications.length > 0) updateCurrentEngineStyles();
});

// --- DEBOUNCING ASINCRONO OPACITA' ---
let opacityUpdateFrame;
document.getElementById('opacitySlider').addEventListener('input', function(e) {
    currentOpacity = parseFloat(e.target.value);
    
    // Cancella frame precedenti se l'utente muove velocemente lo slider
    if (opacityUpdateFrame) cancelAnimationFrame(opacityUpdateFrame);
    
    // Richiede l'aggiornamento grafico al prossimo frame utile del browser
    opacityUpdateFrame = requestAnimationFrame(async () => {
        // Yield asincrono: sblocca temporaneamente l'interfaccia 
...
function buildList() {
    ...
    modifications.forEach(mod => {
        ...
        if (isResolved) {
            li.innerHTML = `<strong style="font-size:12px;">${resi}</strong><br><span style="font-size:9px;color:#7f8c8d;">${typeStr}</span><br><strong style="color:#2c3e50;">${displayMod}</strong>`;
            li.style.borderLeft = `5px solid ${hexColor}`;
            li.title = `Center on residue ${resi}`;
            ...
            li.innerHTML = `<strong>${resi}</strong><br><span style="font-size:9px;color:#7f8c8d;">${typeStr}</span><br>${displayMod} <br><span style="color:#e74c3c; font-size:9px; font-weight:bold;">(Not in 3D)</span>`;
...
// ==========================================
// 3. APPLICAZIONE STILI (TOOLTIP E HOVER RIMOSSI PER PERFORMANCE MAX)
// ==========================================
...
async function apply3DmolStyles() {
    ...
    // Raggruppa (Batching) per minimizzare i comandi
    modifications.forEach(mod => {
        const resi = mod["Positions in the Structure"];
        const isMod = mod["Knwon Positions Modifications"] === "Y"; 
        const typeStr = mod["Type Structure"];
    ...
    // Applica in blocco (Senza Hover!)
    Object.values(styleGroups).forEach(group => {
        viewer3Dmol.setStyle(
            { chain: group.chain, resi: group.resi }, 
            { cartoon: { color: group.color, opacity: 1.0 }, sphere: { radius: 1.2, color: group.color } }
        );
    });
    ...
    modifications.forEach(mod => {
        ...
        if (residueExists(resi, targetAuth) && (isMod || showUnknown)) {
            ...
            selectionData.push({
                ...
                color: paletteObj.rgb, focus: false, representation: 'spacefill', representationColor: paletteObj.rgb
            });
        }
    });
...
function applyJSmolStyles() {
    ...
    Object.values(styleGroups).forEach(group => {
        ...
        script += `select (resno=${group.resi.join(",")} and chain=${group.chain}); color cartoon opaque; color cartoon ${jsmolColor}; spacefill 1.5; color spacefill ${jsmolColor}; `;
    });
    
    script += "select none;"; Jmol.script(myJmol, script);
}
