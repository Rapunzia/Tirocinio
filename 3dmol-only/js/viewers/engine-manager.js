...
function apply3DmolAnnotationStyles() {
    ...
    const isCustomTabActive = appState.activeInspectorTab === 'custom';
    ...
    groups.forEach((group) => {
        ...
        // Filter out hidden residues
        const visibleResi = group.resi.filter(r => !appState.hiddenResidues.has(`${r}|${group.chain}`));
        if (visibleResi.length === 0) return;

        let customResi = [];
        let nonCustomResi = [];
        
        if (isCustomTabActive) {
            visibleResi.forEach(r => {
        ...
        if (labelResStyle) {
            appState.viewer3Dmol.addResLabels({ chain: group.chain, resi: visibleResi }, labelResStyle);
        }
    });
    ...
    if (appState.customColors && appState.customStyles && (appState.customColors.size > 0 || appState.customStyles.size > 0)) {
        ...
        processedKeys.forEach(key => {
            // Skip hidden residues
            if (appState.hiddenResidues.has(key)) return;
